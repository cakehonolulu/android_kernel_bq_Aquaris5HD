	...
	for(;;)
	{
		...
		{	
			...
			if(wk_tsk[cpu]->pid == current->pid)
			{
			   ...
			   printk("[WDK], local_bit:0x%x, cpu:%d, check bit0x:%x,RT[%lld]\n", local_bit, cpu, wk_check_kick_bit(),sched_clock());
			   if(local_bit == wk_check_kick_bit())
			   {
			      printk("[WDK]: kick Ex WDT,RT[%lld]\n",sched_clock());
			   ...
			   spin_unlock(&lock);
               		   ...
               		   #ifdef CONFIG_LOCAL_WDT
			   printk("[WDK]: cpu:%d, kick local wdt,RT[%lld]\n", cpu,sched_clock());
			   // kick local wdt
		...
		if(wk_tsk[cpu]->pid == current->pid)	
		{
		   ...
		   printk("[thread:%d][RT:%lld] %d-%02d-%02d %02d:%02d:%02d.%u UTC; android time %d-%02d-%02d %02d:%02d:%02d.%03d\n",  current->pid,sched_clock(),
