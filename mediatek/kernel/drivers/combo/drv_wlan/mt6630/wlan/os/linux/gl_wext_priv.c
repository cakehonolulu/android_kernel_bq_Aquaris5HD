    ...
    )
{
    ...
    if(pcExtra) {
        ...
        DBGLOG(REQ, INFO, ("priv_driver_cmds i4BytesWritten = %u\n", i4BytesWritten));
    }

    DBGLOG(REQ, INFO, ("pcExtra done\n"));

    /* trick in ./net/wireless/wext-priv.c ioctl_private_iw_point */
    /* becasue the cmd number is even, the return string will not be copy_to_user */

    if(i4BytesWritten > 0) {
        if(!access_ok(VERIFY_WRITE, prIwReqData->data.pointer, i4BytesWritten)) {
            DBGLOG(REQ, INFO, ("priv_driver_cmds access_ok Write fail written = %u\n", i4BytesWritten));
                return  -EFAULT;
         }
    ...
    )
{
    ...
    if(i4CmdFound == 0) {
        ...
        if (strnicmp(pcCommand, CMD_RSSI, strlen(CMD_RSSI)) == 0) {
            ...
            //i4BytesWritten = wl_android_set_suspendmode(net, pcCommand, i4TotalLen);
        }
        else if (strnicmp(pcCommand, CMD_SETBAND, strlen(CMD_SETBAND)) == 0) {
            //uint band = *(pcCommand + strlen(CMD_SETBAND) + 1) - '0';
            //i4BytesWritten = wldev_set_band(net, band);
        }
	...
	if (i4BytesWritten >= 0) {
		...
		if (i4BytesWritten >= i4TotalLen) {
			DBGLOG(REQ, WARN, ("i4BytesWritten = %d\n", i4BytesWritten));
        ...
        )
{
	...
	if ( !access_ok(VERIFY_READ, priv_cmd.buf, i4TotalLen)) {
		DBGLOG(REQ, WARN, ("failed to access_ok len = %u\n", i4TotalLen));
		ret = -ENOMEM;
		goto exit;
    }
	...
	if (!pcCommand)
	{
		DBGLOG(REQ, WARN, ("failed to allocate memory\n"));
		ret = -ENOMEM;
		goto exit;
	}
	...
	if (i4BytesWritten >= 0) {
		...
		if (copy_to_user(priv_cmd.buf, pcCommand, i4BytesWritten)) {
			DBGLOG(REQ, ERROR, ("failed to copy data to user buffer\n"));
			ret = -EFAULT;
		}
        if (copy_to_user(prReq->ifr_data, &priv_cmd, sizeof(priv_driver_cmd_t))) {
			DBGLOG(REQ, ERROR, ("failed to copy driver_cmd to user buffer\n"));
            ret = -EFAULT;
        }
